...
class AABC(BaseAgent):
    ...
    def update_discriminator_helper(self, expert_replay, recent_traj_replay):
        ...
        return {"disc/disc_loss": discriminator_loss.item(), "disc/disc_acc": disc_acc.item()}
    ...
    def update_discriminator(self, expert_replay, recent_traj_replay, n_finished_episodes):
        ...
        if self.discriminator_counter >= math.ceil(1.0 / self.discriminator_update_freq):
            for _ in range(self.discriminator_update_n):
                training_info = self.update_discriminator_helper(expert_replay, recent_traj_replay)
    ...
    def compute_critic_loss(self, samples):
        ...
        values = self.critic(
            samples["obs"], samples["actions"], episode_dones=samples["episode_dones"], save_feature=True
        )
    ...
    def update_parameters(self, memory, updates, expert_replay, with_v=False):
